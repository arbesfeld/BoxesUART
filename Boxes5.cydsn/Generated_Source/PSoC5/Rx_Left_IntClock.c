...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Rx_Left_IntClock_CLKEN |= Rx_Left_IntClock_CLKEN_MASK;
	Rx_Left_IntClock_CLKSTBY |= Rx_Left_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Rx_Left_IntClock_CLKEN &= (uint8)(~Rx_Left_IntClock_CLKEN_MASK);
	Rx_Left_IntClock_CLKSTBY &= (uint8)(~Rx_Left_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_StopBlock(void) 
{
    if ((Rx_Left_IntClock_CLKEN & Rx_Left_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(Rx_Left_IntClock__CFG3)
        CLK_DIST_AMASK = Rx_Left_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Rx_Left_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Rx_Left_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Rx_Left_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Rx_Left_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Rx_Left_IntClock_CLKSTBY &= (uint8)(~Rx_Left_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Rx_Left_IntClock_GetSourceRegister();
    uint16 oldDivider = Rx_Left_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Rx_Left_IntClock_CLKEN & Rx_Left_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Rx_Left_IntClock_DIV_PTR, clkDivider);
                Rx_Left_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Rx_Left_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Rx_Left_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Rx_Left_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Rx_Left_IntClock_DIV_PTR, clkDivider);
				Rx_Left_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Rx_Left_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Rx_Left_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    Rx_Left_IntClock_MOD_SRC |= modeBitMask & (uint8)Rx_Left_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    Rx_Left_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Rx_Left_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Rx_Left_IntClock_GetModeRegister(void) 
{
    return Rx_Left_IntClock_MOD_SRC & (uint8)(Rx_Left_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Rx_Left_IntClock_GetDividerRegister();
    uint8 oldSrc = Rx_Left_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Rx_Left_IntClock_MOD_SRC |= CYCLK_SSS;
        Rx_Left_IntClock_MOD_SRC =
            (Rx_Left_IntClock_MOD_SRC & (uint8)(~Rx_Left_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Rx_Left_IntClock_GetSourceRegister(void) 
{
    return Rx_Left_IntClock_MOD_SRC & Rx_Left_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Rx_Left_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Rx_Left_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    Rx_Left_IntClock_PHASE = clkPhase & Rx_Left_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Rx_Left_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Rx_Left_IntClock_GetPhaseRegister(void) 
{
    return Rx_Left_IntClock_PHASE & Rx_Left_IntClock_PHASE_MASK;
}
