...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Rx_Left) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        Rx_Left_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
        ...
        */
        if(0u != Rx_Left_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = Rx_Left_GetRxBufferSize();
                CyDelay(Rx_Left_BYTE2BYTE_TIME_OUT);
            }
            while(Rx_Left_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(Rx_Left_GetRxBufferSize() > 0u)
    {
        tempCount = Rx_Left_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = Rx_Left_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional time Rx_Left_WAIT_EOP_DELAY.
            */
            if(pData[dataIndexCntr - 1u] != Rx_Left_PACKET_EOP)
            {
                CyDelay(Rx_Left_WAIT_EOP_DELAY);
            }
        }
