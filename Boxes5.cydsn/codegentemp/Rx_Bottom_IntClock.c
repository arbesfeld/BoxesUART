/*******************************************************************************
* File Name: Rx_Bottom_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_Start
********************************************************************************
*
* Summary:
...
void Rx_Bottom_IntClock_Start(void) 
{
    ...
    Rx_Bottom_IntClock_CLKEN |= Rx_Bottom_IntClock_CLKEN_MASK;
	Rx_Bottom_IntClock_CLKSTBY |= Rx_Bottom_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Rx_Bottom_IntClock_Stop(void) 
{
    ...
    Rx_Bottom_IntClock_CLKEN &= (uint8)(~Rx_Bottom_IntClock_CLKEN_MASK);
	Rx_Bottom_IntClock_CLKSTBY &= (uint8)(~Rx_Bottom_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Rx_Bottom_IntClock_CLKEN & Rx_Bottom_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Rx_Bottom_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Rx_Bottom_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Rx_Bottom_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Rx_Bottom_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Rx_Bottom_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Rx_Bottom_IntClock_GetSourceRegister();
    uint16 oldDivider = Rx_Bottom_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Rx_Bottom_IntClock_CLKEN & Rx_Bottom_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Rx_Bottom_IntClock_DIV_PTR, clkDivider);
                Rx_Bottom_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Rx_Bottom_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Rx_Bottom_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Rx_Bottom_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Rx_Bottom_IntClock_DIV_PTR, clkDivider);
				Rx_Bottom_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Rx_Bottom_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Rx_Bottom_IntClock_MOD_SRC |= modeBitMask & (uint8)Rx_Bottom_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Rx_Bottom_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Rx_Bottom_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Rx_Bottom_IntClock_MOD_SRC & (uint8)(Rx_Bottom_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Rx_Bottom_IntClock_GetDividerRegister();
    uint8 oldSrc = Rx_Bottom_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Rx_Bottom_IntClock_MOD_SRC |= CYCLK_SSS;
        Rx_Bottom_IntClock_MOD_SRC =
            (Rx_Bottom_IntClock_MOD_SRC & (uint8)(~Rx_Bottom_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Rx_Bottom_IntClock_MOD_SRC & Rx_Bottom_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Rx_Bottom_IntClock_PHASE = clkPhase & Rx_Bottom_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Rx_Bottom_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Rx_Bottom_IntClock_PHASE & Rx_Bottom_IntClock_PHASE_MASK;
}
