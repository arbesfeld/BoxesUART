...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Rx_Right) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        Rx_Right_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != Rx_Right_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = Rx_Right_GetRxBufferSize();
                CyDelay(Rx_Right_BYTE2BYTE_TIME_OUT);
            }
            while(Rx_Right_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(Rx_Right_GetRxBufferSize() > 0u)
    {
        tempCount = Rx_Right_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = Rx_Right_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional time Rx_Right_WAIT_EOP_DELAY.
            */
            if(pData[dataIndexCntr - 1u] != Rx_Right_PACKET_EOP)
            {
                CyDelay(Rx_Right_WAIT_EOP_DELAY);
            }
        }
