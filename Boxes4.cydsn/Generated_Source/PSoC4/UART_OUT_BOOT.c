...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_OUT) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_OUT_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_OUT_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_OUT_GetRxBufferSize();
                CyDelay(UART_OUT_BYTE2BYTE_TIME_OUT);
            }
            while(UART_OUT_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(UART_OUT_GetRxBufferSize() > 0u)
    {
        tempCount = UART_OUT_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_OUT_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional time UART_OUT_WAIT_EOP_DELAY.
            */
            if(pData[dataIndexCntr - 1u] != UART_OUT_PACKET_EOP)
            {
                CyDelay(UART_OUT_WAIT_EOP_DELAY);
            }
        }
